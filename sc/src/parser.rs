...
peg::parser!{
  grammar lsc() for str {
    ...
    rule changeRule() -> Stmt =
      name:ruleName() _ (changeRuleModifier())* ":"? _ b:block()
      { Stmt::ChangeRule(ChangeRule { name, block: b }) }
    ...
    rule ruleName() -> String = n:$(name() ("-" (name() / number()))*) { n.to_string() }
